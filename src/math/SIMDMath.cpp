...
void SIMDMath::AVX512::sum(uint32_t size, const float first[], const float second[], float result[])
{
        ...
        SIMDMath::AVX::sum(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX512::sub(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::sub(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX512::mul(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::mul(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX512::div(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::div(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX512::sum(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::sum(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX512::sub(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        const __m512 scalarValues { _mm512_set1_ps(scalar) };
        ...
        SIMDMath::AVX::sub(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX512::mul(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        const __m512 scalarValues { _mm512_set1_ps(scalar) };
        ...
        SIMDMath::AVX::mul(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX512::div(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        const __m512 scalarValues { _mm512_set1_ps(scalar) };
        ...
        SIMDMath::AVX::div(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX512::tanh(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::tanh(size - END, data + END, result + END);
}

void SIMDMath::AVX512::tanhDerivative(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };

        SIMDMath::AVX512::tanh(size, data, result);
        ...
        // Trying to use AVX for the remaining part of the array.
        SIMDMath::AVX::tanhDerivative(size - END, data + END, result + END);
}

void SIMDMath::AVX512::ReLU(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        // Trying to use AVX for the remaining part of the array.
        SIMDMath::AVX::ReLU(size - END, data + END, result + END);
}

void SIMDMath::AVX512::ReLUDerivative(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        // Trying to use AVX for the remaining part of the array.
        SIMDMath::AVX::ReLUDerivative(size - END, data + END, result + END);
}

void SIMDMath::AVX512::min(uint32_t size, const float data[], float min, float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::min(size - END, data + END, min, result + END);
}

void SIMDMath::AVX512::max(uint32_t size, const float data[], float max, float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::max(size - END, data + END, max, result + END);
}

void SIMDMath::AVX512::clamp(uint32_t size, const float data[], float min, float max, float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::clamp(size - END, data + END, min, max, result + END);
}

void SIMDMath::AVX512::min(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::min(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX512::max(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::max(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX512::clamp(uint32_t size, const float data[], const float min[], const float max[], float result[])
{
        const uint32_t END { size & ~(AVX512_SIMD_WIDTH - 1) };
        ...
        SIMDMath::AVX::clamp(size - END, data + END, min + END, max + END, result + END);
}

void SIMDMath::AVX512::matrixVectorMul(uint32_t width, uint32_t height, const float matrix[], const float vector[], float result[])
{
...
void SIMDMath::AVX::sum(uint32_t size, const float first[], const float second[], float result[])
{
        ...
        SIMDMath::SSE::sum(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX::sub(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::sub(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX::mul(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::mul(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX::div(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::div(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX::sum(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::sum(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX::sub(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        const __m256 scalarValues { _mm256_set1_ps(scalar) };
        ...
        SIMDMath::SSE::sub(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX::mul(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        const __m256 scalarValues { _mm256_set1_ps(scalar) };
        ...
        SIMDMath::SSE::mul(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX::div(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        const __m256 scalarValues { _mm256_set1_ps(scalar) };
        ...
        SIMDMath::SSE::div(size - END, first + END, scalar, result + END);
}

void SIMDMath::AVX::tanh(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::tanh(size - END, data + END, result + END);
}

void SIMDMath::AVX::tanhDerivative(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };

        SIMDMath::AVX::tanh(size, data, result);
        ...
        // Trying to use SSE for the remaining part of the array.
        SIMDMath::SSE::tanhDerivative(size - END, data + END, result + END);
}

void SIMDMath::AVX::ReLU(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        // Trying to use SSE for the remaining part of the array.
        SIMDMath::SSE::ReLU(size - END, data + END, result + END);
}

void SIMDMath::AVX::ReLUDerivative(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        // Trying to use SSE for the remaining part of the array.
        SIMDMath::SSE::ReLUDerivative(size - END, data + END, result + END);
}

void SIMDMath::AVX::min(uint32_t size, const float data[], float min, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::min(size - END, data + END, min, result + END);
}


void SIMDMath::AVX::max(uint32_t size, const float data[], float max, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::max(size - END, data + END, max, result + END);
}

void SIMDMath::AVX::clamp(uint32_t size, const float data[], float min, float max, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::clamp(size - END, data + END, min, max, result + END);
}

void SIMDMath::AVX::min(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::min(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX::max(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::max(size - END, first + END, second + END, result + END);
}

void SIMDMath::AVX::clamp(uint32_t size, const float data[], const float min[], const float max[], float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
        ...
        SIMDMath::SSE::clamp(size - END, data + END, min + END, max + END, result + END);
}

void SIMDMath::AVX::matrixVectorMul(uint32_t width, uint32_t height, const float matrix[], const float vector[], float result[])
{
...
void SIMDMath::SSE::sum(uint32_t size, const float first[], const float second[], float result[])
{
...
void SIMDMath::SSE::sub(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::mul(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::div(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::sum(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::sub(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
        const __m128 scalarValues { _mm_set1_ps(scalar) };
...
void SIMDMath::SSE::mul(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
        const __m128 scalarValues { _mm_set1_ps(scalar) };
...
void SIMDMath::SSE::div(uint32_t size, const float first[], float scalar, float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
        const __m128 scalarValues { _mm_set1_ps(scalar) };
...
void SIMDMath::SSE::tanh(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::tanhDerivative(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };

        SIMDMath::SSE::tanh(size, data, result);
...
void SIMDMath::SSE::ReLU(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::ReLUDerivative(uint32_t size, const float data[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::min(uint32_t size, const float data[], float min, float result[])
{
        const uint32_t END { size & ~(AVX_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::max(uint32_t size, const float data[], float max, float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::clamp(uint32_t size, const float data[], float min, float max, float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::min(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::max(uint32_t size, const float first[], const float second[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::clamp(uint32_t size, const float data[], const float min[], const float max[], float result[])
{
        const uint32_t END { size & ~(SSE_SIMD_WIDTH - 1) };
...
void SIMDMath::SSE::matrixVectorMul(uint32_t width, uint32_t height, const float matrix[], const float vector[], float result[])
{
